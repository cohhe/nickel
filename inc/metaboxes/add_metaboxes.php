...
class nickel_create_meta_boxes {
	private $nickel_config;
	protected $nickel_options;

	public function __construct($nickel_config, $nickel_options) {
		$this->config = $nickel_config;
		$this->options = $nickel_options;
	...
	public function save_meta_boxes($post_id) {
		...
		if(!wp_verify_nonce($_POST[$this->config['id'] . '_noncename'], 'layout_meta'))
			return $post_id;
			...
			if (isset($option['id']) && ! empty($option['id'])) {
				if(!isset($option['only']) || $option['only'] == $_POST['post_type'] || in_array($_POST['post_type'], explode(',', $option['only']))) {
					if(isset($_POST[$option['id']])) {
						$value = sanitize_key($_POST[$option['id']]);
					} else {
						$value = false;
					}
	...
	public function render() {
		...
		// echo '
		// 		<input type="hidden" name="' . esc_attr($this->config['id']) . '_noncename" id="' . esc_attr($this->config['id']) . '_noncename" value="' . wp_create_nonce(plugin_basename(__FILE__)) . '" />
		// 	</div>';
		wp_nonce_field('layout_meta', $this->config['id']. '_noncename');
	}
